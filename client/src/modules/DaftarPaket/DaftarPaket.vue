...
import EditIcon from '@/modules/DaftarPaket/Icon/EditIcon.vue';
import DetailIcon from '@/modules/DaftarPaket/Icon/DetailIcon.vue';
...
// import widget
import FormAdd from '@/modules/DaftarPaket/Widget/FormAdd.vue';
import FormEdit from '@/modules/DaftarPaket/Widget/FormEdit.vue';

// import component
import DetailPaket from '@/modules/DetailPaket/DetailPaket.vue';
  ...
  <div v-else-if="isFormOpen">
    <FormAdd
      ...
      @status="
        (payload) =>
          displayNotification(
            payload.err_msg || 'Paket gagal ditambahkan',
  ...
  <div v-else-if="isFormOpenEdit">
    <FormEdit
      ...
      @status="
        (payload) =>
          displayNotification(
            payload.err_msg || 'Paket gagal diubah',
            payload.error ? 'error' : 'success',
          )
      "
    />
      ...
      <div class="flex items-center">
        ...
        <div class="flex items-center ml-4">
          ...
          <select
            ...
            @change="fetchData()"
          >
            <option v-for="cabang in optionFilterCabang" :key="cabang.id" :value="cabang.id">
              {{ cabang.name }}
            </option>
          </select>
        </div>
          ...
          <template v-if="dataPaket && dataPaket.length > 0">
            <tr v-for="paket in dataPaket" :key="paket.id" class="hover:bg-gray-50">
              <td
                ...
                :class="paket.tutup_paket == 'tutup' ? ' pointer-events-none opacity-50 ' : ''"
              >
              ...
              >
                <span
                  class="absolute left-1/2 top-full -translate-x-1/2 -translate-y-1/2 bg-white px-4 py-2 rounded shadow text-gray-600"
                >
              ...
              <td
                ...
                :class="paket.tutup_paket == 'tutup' ? ' pointer-events-none opacity-50 ' : ''"
              >
                {{ shortText(paket.description, 200) }}
              </td>
              <td
                class="px-6 py-4 text-center"
                :class="paket.tutup_paket == 'tutup' ? ' pointer-events-none opacity-50 ' : ''"
              >
                ...
                >
                  <strong>{{
                    paket.status === 'belum_berangkat' ? 'BELUM BERANGKAT' : 'SUDAH BERANGKAT'
                  }}</strong>
                </span>
              </td>
              <td
              ...
              <td
                ...
                :class="paket.tutup_paket == 'tutup' ? ' pointer-events-none opacity-50 ' : ''"
              >
                {{ paket.quota_jamaah }} Orang
              </td>
              <td class="px-6 py-4 text-center">
                <div class="flex justify-end gap-2">
                  <LightButton
                    @click="
                      openDetailPaket(paket.id, paket.division_id, paket.division_name, paket.name)
                    "
                  >
