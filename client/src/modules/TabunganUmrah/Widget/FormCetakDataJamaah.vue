...
const validateForm = (): boolean => {
  ...
  if (!form.id) {
    showConfirmation('Konfirmasi', 'ID tabungan tidak ditemukan. Silakan coba lagi.', () => {
      emit('close');
    });
    isValid = false;
  }
...
const cetakDataJamaah = async () => {
  ...
  showConfirmation('Konfirmasi Cetak', 'Anda yakin ingin mencetak data jamaah?', async () => {
    try {
      isLoading.value = true;
      ...
      displayNotification(
        error?.response?.data?.error_msg || 'Terjadi kesalahan dalam mencetak data jamaah',
        'error',
      );
    } finally {
      isLoading.value = false;
...
<template>
  ...
  <div
    v-if="props.isFormCetakDataJamaahOpen && !isLoading"
