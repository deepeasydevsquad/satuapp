...
// Import Icon
import DeleteIcon from '@/modules/Airlines/Icon/DeleteIcon.vue';
import EditIcon from '@/modules/Airlines/Icon/EditIcon.vue';
// import element
import DangerButton from '@/modules/Airlines/Particle/DangerButton.vue';
import Pagination from '@/components/Pagination/Pagination.vue';
import LightButton from '@/components/Button/LightButton.vue';
import FormAddUpdate from '@/modules/Investor/Widget/FormAddUpdate.vue';
...
const fetchData = async () => {
  const response = await daftarInvestorAPI({
    search: search.value,
        ...
        <tbody class="divide-y divide-gray-100 border-t border-gray-100">
          <template v-if="dataInvestor && dataInvestor.length > 0">
            <tr v-for="investor in dataInvestor" :key="investor.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 text-center">
                {{ investor.mobile_phone }}<br />
                {{ investor.address }}
              </td>
              <td class="px-6 py-4 text-center">
                {{ investor.invesment }}<br />
                {{ investor.stock }}% Saham
              </td>
              <td class="px-6 py-4 text-center">{{ investor.cabang }}</td>
              <td class="px-6 py-4 text-center">
                <div class="flex justify-center gap-2">
                  <LightButton @click="editInvestor(investor.id)" class="p-2"
                    ><EditIcon
                  /></LightButton>
          ...
          <tr v-else>
            <td colspan="5" class="px-6 py-3 text-center text-base text-gray-600">
              Daftar investor tidak ditemukan.
            </td>
          </tr>
        </tbody>
