...
<template>
  ...
  >
    ...
    >
      ...
      >
      <div
        class="relative inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-3xl sm:align-middle p-6"
      >
        <!-- Title -->
        ...
        <div class="overflow-y-auto max-h-[64vh] no-scrollbar px-1">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 text-gray-600">
            <div>
              ...
              />
              <p v-if="errors.giver_handover" class="mt-1 text-sm text-red-600">
                {{ errors.giver_handover }}
              </p>
            </div>
            <div>
              <input
                v-model="form.giver_handover_identity"
                type="text"
                placeholder="No Identitas Pemberi Barang"
                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-600 font-normal"
              />
              <p v-if="errors.giver_handover_identity" class="mt-1 text-sm text-red-600">
                {{ errors.giver_handover_identity }}
              </p>
            </div>
            <div>
              <input
                v-model="form.giver_handover_hp"
                type="text"
                placeholder="No HP Pemberi Barang"
                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-600 font-normal"
              />
              <p v-if="errors.giver_handover_hp" class="mt-1 text-sm text-red-600">
                {{ errors.giver_handover_hp }}
              </p>
            </div>
          </div>
          ...
          <div class="mb-4 text-gray-600">
            <textarea
              v-model="form.giver_handover_address"
              rows="3"
              placeholder="Alamat Pemberi Barang"
              class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-600 font-normal"
            ></textarea>
            <p v-if="errors.giver_handover_address" class="mt-1 text-sm text-red-600">
              {{ errors.giver_handover_address }}
            </p>
          </div>
          ...
          <ul class="space-y-2 mb-4 text-gray-600">
            <li v-for="(barang, index) in form.barangList" :key="index" class="flex flex-col gap-1">
              ...
              <p v-if="errors.barangList[index]" class="text-sm text-red-600">
                {{ errors.barangList[index] }}
              </p>
            </li>
          </ul>
          ...
          <button
            @click="addBarang"
            class="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded hover:bg-gray-100 flex items-center gap-2"
            type="button"
          >
