...
import Confirmation from '@/modules/Airlines/Particle/Confirmation.vue';
  ...
  const action = async () => {
    try {
      if (isEdit) {
        ...
        emit('status', {
          error: false,
          err_msg: response?.error_msg || response?.message || 'Maskapai berhasil di update',
        });
      } else {
        ...
        emit('status', {
          error: false,
          err_msg: response?.error_msg || response?.message || 'Maskapai berhasil di tambahkan',
        });
      }
      emit('close');
    } catch (error) {
