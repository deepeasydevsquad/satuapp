  ...
  showConfirmation(
    ...
    async () => {
      ...
      try {
        ...
        emit('status', {
          error: false,
          err_msg: response.data.error_msg || response.data.message || 'Dana berhasil dikembalikan',
        });
        emit('close');
      } catch (error: any) {
        ...
        <div class="overflow-y-auto no-scrollbar px-1">
          <div class="space-y-4 text-gray-800 sm:min-h-[200px]">
            ...
            <div>
              ...
              />
              <p v-if="errors.nominal_refund" class="mt-1 text-sm text-red-600">
                {{ errors.nominal_refund }}
              </p>
            </div>
          </div>
        </div>
