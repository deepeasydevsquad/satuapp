...
import SubmenuItem from '@/modules/TransPaket/Particle/SubmenuItem.vue';
  ...
  <div class="inline-flex rounded-md shadow-xs mb-5" role="group">
    <template v-for="item in items" :key="item.value">
      <button
        ...
        v-if="item.value == 'daftar_paket'"
        type="button"
        :class="
          current === item.value
            ? 'shadow-md z-10 ring-2 text-blue-700 ring-blue-700 bg-gray-200 font-semibold'
      ...
      >
        <font-awesome-icon
          ...
          :class="
            current === item.value
              ? 'text-blue-700 ring-blue-700 font-normal'
              : ' text-gray-600  hover:bg-gray-100'
          "
          class="w-4 h-4 me-1 hover:text-blue-700 focus:text-blue-700"
        />
        <span>{{ item.label }}</span>
      </button>
      <button
        @click="current = item.value"
        v-else-if="item.value == 'pembayaran_paket_agen'"
        type="button"
        :class="
          current === item.value
            ? 'z-10 ring-2 text-blue-700 ring-blue-700 bg-gray-200 shadow-md font-semibold'
            : 'text-gray-700 hover:bg-gray-100'
        "
