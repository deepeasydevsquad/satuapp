...
import { getInfoUpdate, getAgen, getJamaahNotMember, editJamaah } from '@/service/daftar_jamaah';
...
const formData = ref<FormDataState>({
  ...
  tanggal_nikah: null,
  kewarganegaraan: 'wni',
  mahram: [{ mahram_id: null, mst_mahram_type_id: null }],
  keterangan: null,
  address: null,
  provinsi_id: null,
  kabupaten_id: null,
...
const fetchData = async () => {
  ...
  try {
    ...
    dokumen.value = Object.fromEntries(
      docTypes.map((type) => [
        type,
        infoUpdateRes.data.documents.find(
          (doc: { type: string; value: boolean }) => doc.type === type,
        )?.value === true,
      ]),
    );
...
const validateForm = (): boolean => {
  ...
  const fieldValidators: Record<string, ValidatorFn[]> = {
    ...
    birth_date: [
      ...
      isDate('Tanggal lahir'),
      isBeforeToday('Tanggal lahir'),
    ],
...
const photoPreviewUrl = computed(() => {
  ...
  return fileFoto.value ? (fileFoto.value = null) : URL.createObjectURL(formData.value.photo);
});
...
<template>
  ...
  >
    ...
    >
      ...
      >
      <div
        class="relative p-6 inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl sm:align-middle"
      >
        <h1
          class="border-b border-gray-200 pb-4 text-2xl flex items-center justify-center font-bold leading-6 text-gray-900 mb-4"
        >
          Form Edit Jamaah
        </h1>
          ...
          <div class="space-y-3 p-1">
            ...
            <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                <span v-if="errors.title" class="mt-1 text-sm text-red-600">{{
                  errors.title
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                <input
                  ...
                  required
                />
                <span v-if="errors.fullname" class="mt-1 text-sm text-red-600">{{
                  errors.fullname
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                />
                <span v-if="errors.nama_passport" class="mt-1 text-sm text-red-600">{{
                  errors.nama_passport
                }}</span>
              </div>
              <div class="space-y-1">
                <label class="block w-full text-sm font-medium text-gray-700"
                  >Nomor Identitas <span class="text-red-500">*</span></label
                >
                <input
                  ...
                  required
                />
                <span v-if="errors.identity_number" class="mt-1 text-sm text-red-600">{{
                  errors.identity_number
                }}</span>
              </div>
              <div class="space-y-1">
                <label class="block w-full text-sm font-medium text-gray-700"
                  >Jenis Identitas <span class="text-red-500">*</span></label
                >
                <select
                ...
                <span v-if="errors.identity_type" class="mt-1 text-sm text-red-600">{{
                  errors.identity_type
                }}</span>
              </div>
              <div class="space-y-1">
                <label class="block w-full text-sm font-medium text-gray-700"
                  >Kewarganegaraan <span class="text-red-500">*</span></label
                >
                <select
                ...
                <span v-if="errors.kewarganegaraan" class="mt-1 text-sm text-red-600">{{
              ...
              <div class="space-y-1">
                ...
                <span v-if="errors.gender" class="mt-1 text-sm text-red-600">{{
                  errors.gender
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                <span v-if="errors.blood_type" class="mt-1 text-sm text-red-600">{{
                  errors.blood_type
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                <input
                  ...
                  required
                />
                <span v-if="errors.birth_place" class="mt-1 text-sm text-red-600">{{
                  errors.birth_place
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                <input
                  ...
                  required
                />
                <span v-if="errors.birth_date" class="mt-1 text-sm text-red-600">{{
                  errors.birth_date
                }}</span>
              </div>
            </div>
            ...
            <div class="mt-2">
              ...
              <div class="space-y-1 pt-2 pb-2">
                ...
                <textarea
                  ...
                  required
                ></textarea>
                <span v-if="errors.address" class="mt-1 text-sm text-red-600">{{
                  errors.address
                }}</span>
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1">
                  ...
                  <input
                    ...
                    required
                  />
                  <span v-if="errors.kode_pos" class="mt-1 text-sm text-red-600">{{
                    errors.kode_pos
                  }}</span>
                </div>
                <div class="space-y-1">
                  ...
                  <input
                    ...
                    required
                  />
                  <span v-if="errors.nomor_telephone" class="mt-1 text-sm text-red-600">{{
                    errors.nomor_telephone
                  }}</span>
                </div>
                <div class="space-y-1">
                  <label class="block w-full text-sm font-medium text-gray-700"
                  ...
                  <input
                    ...
                    required
                  />
                  <span v-if="errors.whatsapp_number" class="mt-1 text-sm text-red-600">{{
            ...
            <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                />
                <span v-if="errors.nomor_passport" class="mt-1 text-sm text-red-600">{{
                  errors.nomor_passport
                }}</span>
              </div>
              <div class="space-y-1">
                <label class="block w-full text-sm font-medium text-gray-700"
                  >Masa Berlaku Passport</label
                >
                <input
                  ...
                  class="block w-full px-3 py-2 placeholder:text-gray-500 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                />
                <span v-if="errors.masa_berlaku_passport" class="mt-1 text-sm text-red-600">{{
                  errors.masa_berlaku_passport
                }}</span>
              </div>
              <div class="space-y-1">
                <label class="block w-full text-sm font-medium text-gray-700"
                  >Tanggal Dikeluarkan Passport</label
                >
                <input
                ...
                />
                <span
                  v-if="errors.tanggal_di_keluarkan_passport"
                  class="mt-1 text-sm text-red-600"
                  >{{ errors.tanggal_di_keluarkan_passport }}</span
                >
              </div>
              <div class="space-y-1">
                <label class="block w-full text-sm font-medium text-gray-700"
                  >Tempat Dikeluarkan Passport</label
                >
                <input
                ...
                />
                <span
                  v-if="errors.tempat_di_keluarkan_passport"
                  class="mt-1 text-sm text-red-600"
                  >{{ errors.tempat_di_keluarkan_passport }}</span
                >
              </div>
            </div>
            ...
            <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                />
                <span v-if="errors.nama_ayah" class="mt-1 text-sm text-red-600">{{
                  errors.nama_ayah
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                />
                <span v-if="errors.nama_keluarga" class="mt-1 text-sm text-red-600">{{
                  errors.nama_keluarga
                }}</span>
              </div>
              <div class="space-y-1">
                <label class="block w-full text-sm font-medium text-gray-700"
                  >Nomor Telepon Keluarga</label
                >
                <input
                ...
                />
                <span v-if="errors.telephone_keluarga" class="mt-1 text-sm text-red-600">{{
                  errors.telephone_keluarga
                }}</span>
              </div>
              <div class="space-y-1">
                <label class="block w-full text-sm font-medium text-gray-700"
                  >Alamat Keluarga</label
                >
                <textarea
                ...
                ></textarea>
                <span v-if="errors.alamat_keluarga" class="mt-1 text-sm text-red-600">{{
            ...
            >
              ...
              <div class="grid grid-cols-1 md:grid-cols-2">
                ...
                <template v-if="mahramData.mahram_id && !mahramData.mst_mahram_type_id">
                  <p class="text-sm text-red-600">Jenis mahram wajib diisi</p>
                </template>
              </div>
            </div>
            <PrimaryButton
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                <span class="mt-1 text-sm text-red-600" v-if="errors.status_nikah">{{
                  errors.status_nikah
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                />
                <span class="mt-1 text-sm text-red-600" v-if="errors.tanggal_nikah">{{
                  errors.tanggal_nikah
                }}</span>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                <select
                  v-model="formData.pengalaman_haji"
                  class="block w-full px-3 py-2 placeholder:text-gray-500 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                >
                  <option v-for="item in pengalaman" :key="item.id" :value="item.id">
                    {{ item.name }}
                  </option>
                </select>
                <span class="mt-1 text-sm text-red-600" v-if="errors.pengalaman_haji">{{
                  errors.pengalaman_haji
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                <input
                  ...
                  :class="{
                    'opacity-50':
                      formData.pengalaman_haji === 1 || formData.pengalaman_haji === null,
                  }"
                  class="block w-full px-3 py-2 placeholder:text-gray-500 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                  :disabled="formData.pengalaman_haji === 1 || formData.pengalaman_haji === null"
                  placeholder="Tahun Haji"
                />
                <span class="mt-1 text-sm text-red-600" v-if="errors.tahun_haji">{{
                  errors.tahun_haji
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                <select
                  v-model="formData.pengalaman_umrah"
                  class="block w-full px-3 py-2 placeholder:text-gray-500 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                >
                  <option v-for="item in pengalaman" :key="item.id" :value="item.id">
                    {{ item.name }}
                ...
                <span class="mt-1 text-sm text-red-600" v-if="errors.pengalaman_umrah">{{
                  errors.pengalaman_umrah
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                <input
                  ...
                  :class="{
                    'opacity-50':
                      formData.pengalaman_umrah === 1 || formData.pengalaman_umrah === null,
                  }"
                  class="block w-full px-3 py-2 placeholder:text-gray-500 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                  :disabled="formData.pengalaman_umrah === 1 || formData.pengalaman_umrah === null"
                  placeholder="Tahun Umrah"
                />
                <span class="mt-1 text-sm text-red-600" v-if="errors.tahun_umrah">{{
                  errors.tahun_umrah
                }}</span>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                <select
                  v-model="formData.mst_pekerjaan_id"
                  class="block w-full px-3 py-2 placeholder:text-gray-500 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                >
                ...
                <span class="mt-1 text-sm text-red-600" v-if="errors.mst_pekerjaan_id">{{
                  errors.mst_pekerjaan_id
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                <input
                  ...
                  :class="{
                    'opacity-50':
                      formData.mst_pekerjaan_id === 8 || formData.mst_pekerjaan_id === null,
                  }"
                  :disabled="formData.mst_pekerjaan_id === 8 || formData.mst_pekerjaan_id === null"
                  class="block w-full px-3 py-2 placeholder:text-gray-500 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                  placeholder="Nama Perusahaan"
                />
                <span class="mt-1 text-sm text-red-600" v-if="errors.profession_instantion_name">{{
                  errors.profession_instantion_name
                }}</span>
              </div>
              <div class="space-y-1">
                ...
                <span
                  class="mt-1 text-sm text-red-600"
                  v-if="errors.profession_instantion_telephone"
                  >{{ errors.profession_instantion_telephone }}</span
                >
              </div>
              <div class="space-y-1">
                ...
                <span
                  class="mt-1 text-sm text-red-600"
                  v-if="errors.profession_instantion_address"
                  >{{ errors.profession_instantion_address }}</span
                >
              </div>
            </div>
            <!-- Section 9: Kesehatan -->
            ...
            <div class="space-y-1">
              ...
              <textarea
                ...
                maxlength="255"
              ></textarea>
              <span v-if="formData.desease.length >= 255" class="mt-1 text-sm text-red-600"
                >Maksimal 255 karakter</span
              >
              <span v-if="errors.desease" class="mt-1 text-sm text-red-600">{{
                errors.desease
              }}</span>
            </div>
            <!-- Section 10: Agen -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div class="flex items-center gap-2">
                <input
                  ...
                  class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                />
                <label for="fcKK" class="text-sm font-medium text-gray-700"> FC KK </label>
              </div>
              <div class="flex items-center gap-2">
                <input
                  type="checkbox"
                  ...
                  class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                />
                <label for="fcKTP" class="text-sm font-medium text-gray-700"> FC KTP </label>
              </div>
              <div class="flex items-center gap-2">
                <input
                  type="checkbox"
              ...
              <div class="space-y-2">
                ...
                <div class="space-y-1">
                  ...
                  <p
                    ...
                    class="mt-1 text-sm text-red-600"
                  >
                    {{ errors.confirm_password || 'Password tidak sama atau cocok' }}
                  </p>
                </div>
              </div>
              <!-- Section 13: Photo -->
