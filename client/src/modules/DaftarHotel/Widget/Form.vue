  ...
  const action = async () => {
    try {
      if (isEdit) {
        ...
        emit('status', {
          error: false,
          err_msg: response?.error_msg || response?.message || 'Kota berhasil di update',
        });
      } else {
        ...
        emit('status', {
          error: false,
          err_msg: response?.error_msg || response?.message || 'Kota berhasil di tambahkan',
        });
      }
      emit('close');
    } catch (error) {
          ...
          <div class="space-y-4">
            <div>
              ...
              >
                ...
                <option v-for="kota in kotaList" :key="kota.id" :value="kota.name">
                  {{ kota.name }}
                </option>
              </select>
