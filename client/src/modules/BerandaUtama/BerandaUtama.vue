...
// Import components
import EditIcon from '@/modules/BerandaUtama/icon/EditIcon.vue';
import DeleteIcon from '@/modules/BerandaUtama/icon/DeleteIcon.vue';
import FormHeadline from '@/modules/Headline/Widget/FormHeadline.vue';
import SkeletonTable from '@/components/SkeletonTable/SkeletonTable.vue';
import InfoCard from '@/modules/BerandaUtama/widget/InfoCard.vue';
import Pagination from '@/components/Pagination/Pagination.vue';
import Notification from '@/modules/BerandaUtama/particle/Notification.vue';
import Confirmation from '@/modules/BerandaUtama/particle/Confirmation.vue';
...
const deleteHeadlineData = async (id: number) => {
  showConfirmation(
    'Konfirmasi Hapus',
    'Apakah Anda yakin ingin menghapus headline ini?',
    async () => {
      try {
        const res = await deleteHeadline(id);
        displayNotification(
          res.error_msg || res.message || 'Headline berhasil dihapus.',
          'success',
        );
        fetchHeadlineData();
      } catch (error: any) {
...
<template>
  ...
  <div class="p-4 space-y-6 text-gray-800">
    <div class="flex justify-between items-center border-b pb-4 mb-4">
      ...
      <PrimaryButtonLight class="flex items-center gap-2 text-base">
        <font-awesome-icon icon="fa-solid fa-money-bill-alt" class="mr-2" />
        <strong>Saldo Perusahaan :</strong> Rp.
        {{ dataStatusCard.saldo_perusahaan.toLocaleString() }}
      </PrimaryButtonLight>
    </div>
      ...
      <div class="md:col-span-5">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <InfoCard
            title="JAMAAH"
            subtitle="terdaftar"
            :count="dataStatusCard.total_jamaah_terdaftar"
            color="bg-cyan-600"
            icon="user-check"
          />
          <InfoCard
            title="PAKET"
            subtitle="akan berangkat"
            :count="dataStatusCard.total_paket_berangkat"
            color="bg-green-600"
            icon="box-open"
          />
          <InfoCard
            title="JAMAAH"
            subtitle="akan berangkat"
            :count="dataStatusCard.total_jamaah_berangkat"
            color="bg-yellow-500"
            icon="user"
          />
          <InfoCard
            ...
            :count="dataStatusCard.total_tiket_terjual"
            color="bg-red-600"
            icon="ticket"
          />
        </div>
      ...
      <div class="md:col-span-3">
        ...
        <div class="bg-white rounded-lg shadow p-4">
          ...
          <div class="overflow-x-auto rounded-lg border">
            <SkeletonTable
              v-if="headline.isLoading"
              :columns="headline.totalColumns"
              :rows="headline.itemsPerPage"
            />
            <table v-else class="w-full text-sm">
              ...
              <tbody>
                <tr
                  v-for="(item, index) in headline.data"
                  :key="index"
                  ...
                  <td class="p-2 align-top">
                    <div class="flex justify-center items-center gap-2">
                      <button
                        ...
                        class="p-2 text-gray-500 bg-gray-100 rounded-md hover:bg-blue-100 hover:text-blue-500 transition-colors"
                      >
                        <EditIcon class="w-4 h-4" />
                      </button>
                      <button
                        @click="deleteHeadlineData(item.id)"
                ...
                <tr v-if="headline.data.length === 0">
                  <td :colspan="headline.totalColumns" class="p-4 text-center text-gray-500">
                    Daftar Headline Tidak Ditemukan
                  </td>
                </tr>
              </tbody>
              <tfoot class="bg-gray-100 font-bold">
                <Pagination
                  ...
                  @page-now="
                    (page: number) =>
                      handlePageChange({ ...headline, fetchData: fetchHeadlineData }, page)
                  "
                />
              </tfoot>
            </table>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="bg-white rounded shadow p-4">
        ...
        <div class="overflow-x-auto rounded-lg border">
          <SkeletonTable
            v-if="jamaah.isLoading"
            :columns="jamaah.totalColumns"
            :rows="jamaah.itemsPerPage"
          />
          <table v-else class="w-full text-sm">
            <thead class="bg-gray-100">
              <tr>
            ...
            <tbody>
              <tr
                v-for="(item, index) in jamaah.data"
                :key="index"
                class="hover:bg-gray-50 border-b"
              >
              ...
              <tr v-if="jamaah.data.length === 0">
                <td :colspan="jamaah.totalColumns" class="p-4 text-center text-gray-500">
            ...
            <tfoot class="bg-gray-100 font-bold">
              <Pagination
                ...
                @page-now="
                  (page: number) =>
                    handlePageChange({ ...jamaah, fetchData: fetchJamaahData }, page)
                "
              />
            </tfoot>
          </table>
      ...
      <div class="bg-white rounded shadow p-4">
        ...
        <div class="overflow-x-auto rounded-lg border">
          <SkeletonTable
            v-if="deposit.isLoading"
            :columns="deposit.totalColumns"
            :rows="deposit.itemsPerPage"
          />
          <table v-else class="w-full text-sm">
            <thead class="bg-gray-100">
              <tr>
            ...
            <tbody>
              <tr
                v-for="(item, index) in deposit.data"
                :key="index"
                class="hover:bg-gray-50 border-b"
              >
                <td class="p-3 align-top">
                ...
                <td class="p-2 align-top">
                  <p>{{ item.bank_info }}</p>
                  <span
                    v-if="item.sending_payment_status === 'sudah_dikirim'"
                    class="px-2 py-0.5 text-xs font-medium text-blue-800 bg-blue-100 rounded-full"
                  >
                    {{ item.sending_payment_status.replace('_', ' ') }}
                  </span>
                  <span
                    v-else
                    class="px-2 py-0.5 text-xs font-medium text-yellow-800 bg-yellow-100 rounded-full"
                  >
                    {{ item.sending_payment_status.replace('_', ' ') }}
                  </span>
                </td>
            ...
            <tfoot class="bg-gray-100 font-bold">
              <Pagination
                ...
                @page-now="
                  (page: number) =>
                    handlePageChange({ ...deposit, fetchData: fetchDepositMemberData }, page)
                "
              />
            </tfoot>
          </table>
  ...
  <FormHeadline
    ...
    @status="
      (payload: any) =>
        displayNotification(
          payload.err_msg || 'Data gagal disimpan',
