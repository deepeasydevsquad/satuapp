...
const terbilangUang = (amount: number, withSen: boolean = false): string => {
  ...
  const toWords = (n: number): string => {
    ...
    if (n < 100)
      return `${toWords(Math.floor(n / 10))} puluh${n % 10 ? ' ' + toWords(n % 10) : ''}`;
    if (n < 200) return `seratus${n > 100 ? ' ' + toWords(n - 100) : ''}`;
    if (n < 1000)
      return `${toWords(Math.floor(n / 100))} ratus${n % 100 ? ' ' + toWords(n % 100) : ''}`;
    if (n < 2000) return `seribu${n > 1000 ? ' ' + toWords(n - 1000) : ''}`;
    if (n < 1_000_000)
      return `${toWords(Math.floor(n / 1000))} ribu${n % 1000 ? ' ' + toWords(n % 1000) : ''}`;
    if (n < 1_000_000_000)
      return `${toWords(Math.floor(n / 1_000_000))} juta${n % 1_000_000 ? ' ' + toWords(n % 1_000_000) : ''}`;
...
<template>
  ...
  <div
    ...
    class="flex flex-col justify-center items-center h-screen bg-red-50 p-4 no-print"
  >
  ...
  >
        ...
        <tfoot>
          <tr class="font-bold bg-gray-100">
            ...
            <td class="border border-black p-2 text-right">
              Rp {{ formatCurrency(data.harga_costumer * data.pax) }}
            </td>
          </tr>
        </tfoot>
      </table>
    ...
    <div class="mb-8 text-sm">
      <p class="font-semibold italic">
        Terbilang: {{ terbilangUang(data.harga_costumer * data.pax) }}
      </p>
    </div>
