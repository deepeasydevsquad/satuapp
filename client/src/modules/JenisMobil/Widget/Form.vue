...
import { addJenisMobil, editJenisMobil } from '@/service/jenis_mobil';
...
const JenisMobil = ref<Partial<JenisMobil>>({
  name: '',
});
...
const saveData = async () => {
  ...
  const isEdit = !!JenisMobil.value.id;
  const action = async () => {
    try {
      if (isEdit) {
        const response = await editJenisMobil(JenisMobil.value.id, JenisMobil.value);
        showConfirmDialog.value = false;
        emit('status', {
          error: false,
          err_msg:
            response?.error_msg || response?.data?.message || 'Jenis Mobil berhasil di update',
        });
      } else {
        const response = await addJenisMobil(JenisMobil.value);
        showConfirmDialog.value = false;
        emit('status', {
          error: false,
          err_msg:
            response?.error_msg || response?.data?.message || 'Jenis Mobil berhasil di tambahkan',
        });
      }
      emit('close');
    } catch (error) {
...
onMounted(() => {
  JenisMobil.value = { ...JenisMobil.value, ...(props.selectedJenisMobil ?? {}) };
});
</script>
      ...
      >
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <h3 class="text-2xl flex justify-center font-bold leading-6 text-gray-900 mb-4">
            {{ JenisMobil.id ? 'Edit Data Jenis Mobil' : 'Tambah Jenis Mobil Baru' }}
          </h3>
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Nama</label>
              <input
                v-model="JenisMobil.name"
                type="text"
        ...
        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
          <PrimaryButton @click="saveData">
            {{ JenisMobil.id ? 'Simpan Perubahan' : 'Tambah' }}
          </PrimaryButton>
          <button
            @click="emit('close')"
