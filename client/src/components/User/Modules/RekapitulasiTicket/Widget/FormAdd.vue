...
const submitForm = async () => {
  ...
  showConfirmation('Konfirmasi', 'Apakah Anda yakin ingin menyimpan rekapitulasi ini?', async () => {
    try {
    ...
    <div class="relative w-full max-w-4xl mx-auto my-8 bg-white rounded-lg shadow-xl">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b">
        ...
        <button @click="closeForm" class="text-gray-400 hover:text-gray-700 transition">
          ...
          <font-awesome-icon icon="fa-solid fa-times" aria-hidden="true" style="font-size: 1.5rem;"></font-awesome-icon>
        </button>
      </div>
      ...
      <div class="px-6 mt-2">
        <div class="overflow-hidden border rounded-lg max-h-[300px]">
          <div class="overflow-y-auto max-h-[300px] no-scrollbar">
            <table v-if="availableTickets.length" class="w-full text-sm text-left text-gray-700 bg-white">
              ...
              <tbody>
                <tr v-for="ticket in availableTickets" :key="ticket.id" class="border-t">
                  <td class="px-4 py-3 align-top hover:bg-gray-100 font-medium">{{ ticket.nomor_register }}</td>
                    ...
                    <div v-for="detail in ticket.ticket_details" :key="detail.id" class="mb-2">
                      <div class="p-2 bg-gray-50 border rounded flex flex-col gap-1">
                        <div class="flex items-center justify-between text-sm font-semibold">
                          <span>
                            <span class="px-2 py-0.5 mr-2 bg-blue-100 text-blue-700 rounded">{{ detail.code_booking }}</span>
                        ...
                        <div class="flex flex-wrap gap-2 mt-1 text-xs text-gray-600">
                          <span v-if="detail.airlines_name" class="inline-flex items-center gap-1">
                            <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M2.5 19.5L21.5 4.5M2.5 4.5l19 15M12 2v20" /></svg>
                          ...
                          <span class="font-bold text-green-600">{{ formatPrice(detail.costumer_price) }}</span>
                        </div>
                      </div>
                    </div>
                    <div class="flex justify-end mt-2">
                      <div class="flex items-center gap-2 px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-sm font-semibold text-gray-700">
                        <span class="text-xs text-gray-500">TOTAL</span>
                        <span class="text-md font-bold text-indigo-700">{{ formatPrice(ticket.total_transaksi) }}</span>
                      </div>
                    </div>
                  </td>
                  <td class="px-4 py-3 align-top hover:bg-gray-100">{{ ticket.tanggal_transaksi }}</td>
                  <td class="px-4 py-3 align-top hover:bg-gray-100 justify-center">
                    ...
                    >
                      <font-awesome-icon icon="fa-solid fa-plus" class="text-xl" :class="{'text-gray-400': selectedTickets.some(t => t.id === ticket.id)}" />
                    </LightButton>
                  </td>
                </tr>
              </tbody>
            </table>
            <p v-else class="py-4 text-center text-gray-500">Tidak ada tiket yang tersedia atau cocok dengan pencarian.</p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 gap-8 px-6 mt-6 md:grid-cols-2">
        <div class="space-y-4 text-gray-800">
          ...
          <div>
            <label for="costumerName" class="block text-sm font-medium text-gray-700">Nama Kostumer <span class="text-red-600">*</span></label>
            <input
              id="costumerName"
            ...
            />
            <p v-if="errors.costumer_name" class="mt-1 text-sm text-red-600">{{ errors.costumer_name }}</p>
          </div>
          <div>
            <label for="costumerWhatsapp" class="block text-sm font-medium text-gray-700">No. WhatsApp <span class="text-red-600">*</span></label>
            <input
              id="costumerWhatsapp"
              ...
              class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
            />
            <p v-if="errors.costumer_whatsapp_number" class="mt-1 text-sm text-red-600">{{ errors.costumer_whatsapp_number }}</p>
          </div>
        </div>
          ...
          <div class="overflow-hidden border rounded-lg max-h-[250px]">
            <div class="overflow-y-auto max-h-[250px] no-scrollbar">
              <table class="w-full text-sm text-left text-gray-700 bg-white">
                ...
                <tbody>
                  <tr v-if="selectedTickets.length === 0">
                    <td colspan="3" class="py-4 text-center text-gray-500">Data rekap tiket tidak ditemukan.</td>
                  </tr>
              ...
              <div v-if="selectedTickets.length" class="px-4 py-2 text-right bg-gray-50">
                ...
                <span class="font-semibold text-gray-800">{{ formatPrice(totalRekapitulasi) }}</span>
              </div>
            </div>
          </div>
        </div>
