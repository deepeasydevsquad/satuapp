...
<template>
  ...
  >
    <div class="flex items-center justify-center gap-2 px-6 py-2.5 lg:py-3.5">
      <router-link to="/">
        <img :src="'/logo-amra.png'" alt="Logo" class="h-10" />
      </router-link>
              ...
              >
                <ul class="mt-4 mb-5.5 flex flex-col gap-2.5 pl-6">
                  <li v-for="(item1, keys) in menu_info?.submenu[item.id]" :key="keys">
                    <router-link
                      :to="''"
                      class="group relative flex items-center gap-2.5 rounded-md px-4 my-2 font-medium text-gray-500 duration-300 ease-in-out hover:text-gray-400"
                      :class="{ '!text-white': item1.name === sidebarStore.selected }"
