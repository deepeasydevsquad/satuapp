<template>
  <Form :form-status="isOpen" :label="'Update Grup'" width="sm:w-full sm:max-w-md" @close="closeModal" @cancel="closeModal"  @submit="saveGroup" :submitLabel="'UPDATE GRUP'">
    <div class="py-6 space-y-4" :key="selectedMenus.length">
      ...
      <div>
        ...
        <select v-model="groupData.branchName" class="w-full border rounded-md px-3 py-2 text-gray-700 bg-white" >
...
watch(
  ...
  async (newVal) => {
    if (newVal) {
      ...
      selectedSubmenus.value = newVal.group_access?.flatMap((menu) => menu.Submenus?.map((submenu) => submenu.id)) || []
