  ...
  const action = async () => {
    try {
      if (isEdit) {
        ...
        emit('status', { error: false, err_msg: (response?.error_msg || response?.data?.message) || 'Jenis Mobil berhasil di update' });
      } else {
        const response = await addJenisMobil(JenisMobil.value);
        showConfirmDialog.value = false;
        emit('status', { error: false, err_msg: (response?.error_msg || response?.data?.message) || 'Jenis Mobil berhasil di tambahkan' });
      }
      emit('close');
    } catch (error) {
