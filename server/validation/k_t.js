const { Jurnal, Paket, Division, Op } = require("../models");
...
validation.check_paket_id = async ( value,  { req } ) => {
    const company_id = await getCompanyIdByCode(req);
    var check = await Paket.findOne({where: { id : value }, include: {
        required: true, 
        model: Division, 
        where : {
    ...
    if (!check) {
        throw new Error("Paket Tidak Ditemukan Dipangkalan Data");
    }
    return true;
}
